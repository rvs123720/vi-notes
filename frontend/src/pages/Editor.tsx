...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    if (id) {
      axios.get(`https://vi-notes-4-7ic7.onrender.com/api/notes/${id}`, {
        headers: { 
  ...
  const saveNote = async () => {
    ...
    try {
      if (id) {
        await axios.put(`https://vi-notes-4-7ic7.onrender.com/api/notes/${id}`, payload, config);
      } else {
        await axios.post("https://vi-notes-4-7ic7.onrender.com/api/notes", payload, config);
      }
      navigate("/dashboard");
    } catch (err: any) {
